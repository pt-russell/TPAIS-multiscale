...
% ****Could move this material to stochastic network section****
...
% ********Did I mean to reintroduce this last paragraph?********** NOOOOO!!!
...
% \cite{parno2018transport} we aim to minimise the Kullback-Liebler (KL)
...
% \begin{equation}D_\text{KL}(\pi_1\|\pi_2) = 
% 		\mathbb{E}_{\pi_1}\left[\log\left(\frac{\pi_1(\theta)}{\pi_2(\theta)}\right)\right].\end{equation}
...
% Each component of the map is defined as a multivariate polynomial expansion,
% \begin{equation}\label{eq:map_defn}
% 	\tilde{T}_i(\theta; \gamma_i) = \sum\limits_{\mathbf{j}\in\mathcal{J}_i} \!
% \gamma_{i,\mathbf{j}}\psi_\mathbf{j}(\theta).
% \end{equation}
...
% in~\cite{parno2018transport}. Increased information with a slower increase in the number of map parameters can be achieved with the composition of maps discussed in~\cite{parno2015transport}.
...
in~\cite{parno2018transport} that this method usually converges in around 10-15 iterations, and we
have seen no evidence that this is not a reasonable estimate. When calculating the map several times
during a Monte Carlo run, using previous guesses of the optimal map to
seed the Newton algorithm 
...
\For{$k \leftarrow 1, \dots, N-1$}{
	Compute $r_i = \tilde{T}(\theta^{(k)}_i; \bar{\gamma}^{(k)})$, \quad $i = 1,\dots,M$.\;
	Sample \edit{$\hat{r}_i \sim q_r(\cdot; r_i, \beta)$.\;
	Invert $\hat{\theta}_i^{(k)} = \tilde{T}^{-1}(\hat{r}_i; \bar{\gamma}^{(k)})$.\;
	Calculate:
	\begin{equation}
		w_i^{(k)} =
                \frac{\pi(\hat{\theta}_i^{(k)})}{\left(\sum_{j=1}^M \!
                    \edit{q_r(\hat{r}_i; r_j, \beta)}\right)|J_{\tilde{T}}(\hat{\theta}_i^{(k)};\bar{\gamma}^{(k)})|}.
	\end{equation}
...
The second option is given in Algorithm~\ref{alg:TransportETAIS2}. We
denote the ensembles of states in target space $\theta^{(k)} =
\{\theta^{(k)}_1,\dots,\theta^{(k)}_M\}$, and the states in the
reference space, \edit{$r = \{r_1,\dots,r_M\}$}, where $M$ is the ensemble
size. Similarly, the proposal states are denoted $\hat{r} =
\{\hat{r}_1,\dots,\hat{r}_M\}$ and $(w^{(k)}, \hat{\theta}^{(k)}) =
\{(w^{(k)}_1, \hat{\theta}^{(k)}_1),\dots,(w^{(k)}_M,
\hat{\theta}^{(k)}_M)\}$, where these \edit{pairs constitute the
  weighted samples} from the target distribution. As in the standard version of the ETAIS algorithm, we use the deterministic mixture weights.
...
\begin{figure}[htpb]
\centering
\subfigure[Original sample $\theta$ from MH-RW algorithm.]{\includegraphics[width=0.31\textwidth]{"images/R2_transport_orig"}}\quad
\subfigure[Pushforward of $\theta$ onto reference space.]{\includegraphics[width=0.31\textwidth]{"images/R2_transport_ref"}}\quad
\subfigure[Pullback of reference sample onto target space.]{\includegraphics[width=0.31\textwidth]{images/R2_transport_pullback}}
...
options.]{\includegraphics[width=0.45\textwidth]{images/R1_L2}}
\subfigure[Comparison of Transport ETAIS option 2 with the standard algorithms.]{\includegraphics[width=0.45\textwidth]{images/R1_L2_all}}
...
\begin{equation}\alpha_j(X(t)) = \lim_{dt \to 0} \mathbb{P}(\text{Reaction $R_j$ in
    the time interval  } \, s \in [t, t+ dt] ).\end{equation} % If a system
...
% However, if the system is stiff, i.e. there
...
% when we include this $\log$ preconditioner leading to an intermediate space $\mathcal{Y}$. 
...
\begin{figure}[!htb]
\centering
% \subfigure[Sampling algorithms with no preconditioner for
    %     $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/CMA_L2_param_space"}}
...
% no log
% preconditioner.]{\includegraphics[width=0.8\textwidth]{"images/CMA_ref_space_MHTRW"}}
% \\
\subfigure[Pushforward of the posterior sample through $\hat{T}$ with log preconditioner.]{\includegraphics[width=0.8\textwidth]{"images/CMA_ref_space_MHlogTRW"}}
...
\edit{At this point, we note that if the transport map found is equal
  ...
  from the pushforward of the target distribution. % ****HIGHLIGHT THIS
...
% ****GREATLY REDUCE THIS SECTION****
% ****Jump straight to Bayes' factors and Figure 7.5****
...
% dependent on all four parameters $\theta_0 = (k_1, k_2, k_3, k_4)^\top$. 
...
\begin{figure}[!htb]
\centering
% \subfigure[Sampling algorithms with no preconditioner for $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/GRN_convergence"}}
...
% presented in \cite{parno2018transport}, where we aim to find an
