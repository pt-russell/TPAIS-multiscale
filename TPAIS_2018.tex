...
\ref{sec:multi}, and in more detail in ****CITE****. These effective
...
which is often the case****CITE DIF ZERO PAPERS****. For this very
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Top: Couplings between parameter space and reference
          when using the transport map. Bottom: Couplings between parameter space and reference space
          when using the transport map proposal with a $\log$ preconditioner.}
...
\caption{Push forward of the target measure on reference space for the problem described in Section~\ref{sec:multiscale_posterior} found using the TRW and logTRW proposal distributions. Components are linked by the relation $r_i = T_i'(k_i)$ in (a) and $r_i = T_i\circ\log(k_i)$ in (b).}
...
interested readers can see more details in ****CITE THESIS****.
...
space. Since, on reference space, the dimensions are uncorrelated, we
