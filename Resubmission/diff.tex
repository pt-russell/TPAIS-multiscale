...
%DIF DEL ../TETAIS.tex   Fri Jan 18 12:54:14 2019
%DIF ADD TETAIS.tex      Tue Jan 14 15:40:05 2020
%test change
\usepackage{cite}
...
%DIF 61a61
\newcommand{\edit}[1]{{\color{red} #1}}  %DIF > 
%DIF -------
...
proposed, including the work of Marzouk and Parno\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , who
...
is used in situations where ODE or diffusion\mbox{%DIFAUXCMD
...
\cite{gillespie1977exact}}\hspace{0pt}%DIFAUXCMD
...
}\DIFdelend %DIF >  In this paper, we particularly concern ourselves with the subject of
%DIF >  inverse problems in stochastic reaction networks. This type of model
%DIF >  is used in situations where ODE or diffusion\cite{gillespie2000chemical}
%DIF >  approximations of chemical reaction kinetics are not viable, usually
%DIF >  because of the low concentration of one or more chemical
%DIF >  species\cite{gillespie1977exact}. Biochemical networks typically fall into
%DIF >  this category, because of the small volume of the reactors involved (a
%DIF >  single cell).

\DIFdelbegin \DIFdel{A range of numerical methods have been suggested to tackle this second
problem, including tau-leaping\mbox{%DIFAUXCMD
\cite{rathinam2003stiffness,cao2006efficient}}\hspace{0pt}%DIFAUXCMD
, diffusion approximations\mbox{%DIFAUXCMD
\cite{erban2006gene,cotter2011constrained,cotter2016error}}\hspace{0pt}%DIFAUXCMD
, quasi-steady state
approximations\mbox{%DIFAUXCMD
\cite{weinan2007nested,cao2005slow}}\hspace{0pt}%DIFAUXCMD
, and other averaging
arguments\mbox{%DIFAUXCMD
\cite{cotter2016constrained}}\hspace{0pt}%DIFAUXCMD
...
types of fast subsystems\mbox{%DIFAUXCMD
\cite{jahnke2007solving,anderson2010product,anderson2016product}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend %DIF >  One challenge with networks arising from these applications is that
%DIF >  they often have some reactions which are occurring many times on a
%DIF >  timescale in which other reactions in the system are unlikely to
%DIF >  occur. This multiscale behaviour throws up two main challenges, both
%DIF >  in observability in an experimental setting, but also in the context
%DIF >  of modelling, where multiscale systems can make the simulation of the
%DIF >  continuous time Markov chain computationally intractable.
...
}\DIFdelend %DIF >  A range of numerical methods have been suggested to tackle this second
%DIF >  problem, including tau-leaping\cite{rathinam2003stiffness,cao2006efficient}, diffusion approximations\cite{erban2006gene,cotter2011constrained,cotter2016error}, quasi-steady state
%DIF >  approximations\cite{weinan2007nested,cao2005slow}, and other averaging
%DIF >  arguments\cite{cotter2016constrained}. Some of these approaches are more
%DIF >  computationally intensive than others, but this can be mitigated with the
%DIF >  use of analytical results about the invariant densities of certain
%DIF >  types of fast subsystems\cite{jahnke2007solving,anderson2010product,anderson2016product}.
...
}\emph{\DIFdel{sloppiness}}%DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\cite{gutenkunst2007universally,apgar2010sloppy} }\hspace{0pt}%DIFAUXCMD
of the likelihood is a challenging problem
that appears in many applications in science and engineering\mbox{%DIFAUXCMD
\cite{constantine2014active}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend %DIF >  We would not expect the fast processes in such a system to be
%DIF >  experimentally observable. In such a situation where only the slow
%DIF >  variables in a system are identifiable, in an ideal world, we would be
%DIF >  be able to integrate over the space of all possible trajectories of
%DIF >  the fast processes. Since this is an insurmountable computational
%DIF >  task, we might instead wish to approximate the likelihoods arising
%DIF >  from such a set of observations by the use of an appropriate
%DIF >  multiscale methodology.

%DIF >  ****Could move this material to stochastic network section****
\DIFaddbegin 



%DIF >  The partial observability of these systems lead to only
%DIF >  partial-observability of the unknown parameters in the system, for
%DIF >  example the reaction rates of the fast reactions. The resulting
%DIF >  multiscale approximations, and the resulting approximations of the
%DIF >  likelihoods arising from the inverse problem for such a network, 
%DIF >  often exhibit 
%DIF >  insensitivity to changes in certain directions of
%DIF >  parameter space. This causes problems when aiming to sample from such
%DIF >  a target, since the density is concentrated on a manifold whose
%DIF >  tangential vectors point in directions of insensitivity. This
%DIF >  \emph{sloppiness}\cite{gutenkunst2007universally,apgar2010sloppy} of the likelihood is a challenging problem
%DIF >  that appears in many applications in science and engineering\cite{constantine2014active}.

%DIF >  ********Did I mean to reintroduce this last paragraph?********** NOOOOO!!!
...
\BlankLine
	...
	\DIFdelbeginFL %DIFDELCMD < \For{$k=1,\ldots,N$}{
%DIFDELCMD < 		Sample $\hat{\theta}_i^{(k)} \sim q(\cdot; \theta_j^{(k)}, \beta)$, for $i = 1,\dots, M$.\label{algline:ETAIS_propose}\;
%DIFDELCMD < 		Calculate $w^{(k)} = (w_1^{(k)}, \ldots, w_M^{(k)})^\top$, where
%DIFDELCMD < 			\[
%DIFDELCMD < 				w^{(k)}_i = \frac{\pi(\hat{\theta}^{(k)}_i)}{\chi^{(k)}(\hat{\theta}^{(k)}_i; \theta^{(k)}, \beta)}.
%DIFDELCMD < 			\]
%DIFDELCMD < 		\label{algline:ETAIS_weights}
%DIFDELCMD < 

%DIFDELCMD < 		Resample $\theta^{(k+1)} \leftarrow \| w^{(k)}\|_1^{-1}\sum_{j=1}^M w_j^{(k)}\delta_{\hat{\theta}_j^{(k)}}(\cdot)$.\label{algline:ETAIS_resample}\;
%DIFDELCMD < 	}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \For{$k=1,\ldots,N$}{
		Sample $\hat{\theta}_i^{(k)} \sim q(\cdot; \theta_i^{(k)}, \beta)$, for $i = 1,\dots, M$.\label{algline:ETAIS_propose}\;
...
methodology was adapted in~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport} }\hspace{0pt}%DIFAUXCMD
...
\cite{parno2018transport} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend for approximately coupling the target
...
\begin{dfn}[Transport Map $T$]
	...
	\DIFdelbegin \[
		\DIFdel{|J_T(\theta)| = \text{det}\begin{bmatrix} \partial_{\theta_1} T_1(\theta) & \dots & \partial_{\theta_d} T_1(\theta) \\ \vdots & \ddots & \vdots \\ \partial_{\theta_1} T_d(\theta) & \dots & \partial_{\theta_d} T_d(\theta) \end{bmatrix}.
	}\]
%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
		\DIFadd{|J_T(\theta)| = \text{det}\begin{bmatrix} \partial_{\theta_1} T_1(\theta) & \dots & \partial_{\theta_d} T_1(\theta) \\ \vdots & \ddots & \vdots \\ \partial_{\theta_1} T_d(\theta) & \dots & \partial_{\theta_d} T_d(\theta) \end{bmatrix}.
	}\end{equation}
\DIFaddend \end{dfn}
...
%DIF <  \cite{parno2014transport} we aim to minimise the Kullback-Liebler (KL)
%DIF >  \cite{parno2018transport} we aim to minimise the Kullback-Liebler (KL)
...
%DIF <  \[D_\text{KL}(\pi_1\|\pi_2) = 
%DIF <  		\mathbb{E}_{\pi_1}\left[\log\left(\frac{\pi_1(\theta)}{\pi_2(\theta)}\right)\right].\]
%DIF >  \begin{equation}D_\text{KL}(\pi_1\|\pi_2) = 
%DIF >  		\mathbb{E}_{\pi_1}\left[\log\left(\frac{\pi_1(\theta)}{\pi_2(\theta)}\right)\right].\end{equation}
...
As in previous work in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , we can ensure
invertibility if we restrict the map to be \DIFdelbegin \DIFdel{lower triangular}\DIFdelend \DIFaddbegin \edit{lower triangular\cite{bogachev2005triangular}}\DIFaddend , i.e. $\tilde{T} \in \mathcal{T}^{\ltri}\subset\mathcal{T}^\uparrow$. \DIFaddbegin \edit{There are a number of different
  approaches to guaranteeing monotonicity\cite{y.17:_handb_uncer_quant}, but here we follow
  the approach taken in \cite{parno2018transport}.} \DIFaddend This lower triangular map has the form,
\DIFdelbegin \[
	\DIFdel{\tilde{T}(\theta_1, \dots, \theta_n) = \begin{bmatrix} T_1(\theta_1) \\ T_2(\theta_1, \theta_2) \\ \vdots \\
		T_n(\theta_1, \dots, \theta_n) \end{bmatrix},
}\]
%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
	\DIFadd{\tilde{T}(\theta_1, \dots, \theta_n) = \begin{bmatrix} T_1(\theta_1) \\ T_2(\theta_1, \theta_2) \\ \vdots \\
		T_n(\theta_1, \dots, \theta_n) \end{bmatrix},
}\end{equation}
\DIFaddend where $T_i\colon \mathbb{R}^i \to \mathbb{R}$. % We assume that the target and reference probability densities are absolutely continuous on
...
\DIFdelend %DIF >  Each component of the map is defined as a multivariate polynomial expansion,
%DIF >  \begin{equation}\label{eq:map_defn}
%DIF >  	\tilde{T}_i(\theta; \gamma_i) = \sum\limits_{\mathbf{j}\in\mathcal{J}_i} \!
%DIF >  \gamma_{i,\mathbf{j}}\psi_\mathbf{j}(\theta).
%DIF >  \end{equation}
%DIF >  The parameter $\gamma_i \in \mathbb{R}^{M_i}$ is a vector of coefficients. Each component of $\gamma_i$ corresponds to a basis function
%DIF >  $\psi_\mathbf{j}$, indexed by the multi-index $\mathbf{j} \in
%DIF >  \mathcal{J}_i \subset \mathbb{N}_0^i$. A multi-index defines a product of univariate polynomials in $\theta_k$,
%DIF >  \begin{equation}
%DIF >  	\psi_\mathbf{j}(\theta) = \prod\limits_{k=1}^i \! \varphi_{j_k}(\theta_k), \quad \text{for} \quad \mathbf{j} \in \mathcal{J}_i,
%DIF >  \end{equation}
%DIF >  and where $\varphi_{j_k}(\theta_k) \in \mathcal{P}(\theta_k)$. Since $\tilde{T}$ is lower triangular, a multi-index $\mathbf{j}\in\mathcal{J}_i$ only contains entries for univariate polynomials in $\theta_k$ for $k\leq i$.
...
}\DIFdelend %DIF >  The cardinalities of the multi-index sets, $M_i = \text{card}(\mathcal{J}_i)$, give the number of unknowns in our
%DIF >  optimisation problem, and so we would like to keep this number as small as possible. One option is
%DIF >  to use polynomials of total order $p$,
%DIF >  \begin{equation}
%DIF >  	\mathcal{J}_i^\text{TO} = \left\{\mathbf{j}:\|\mathbf{j}\|_1 \leq p, j_k = 0\ \forall k > i\right\},
%DIF >  \end{equation}
%DIF >  which is optimal in terms of the amount of information captured by the map about the target. The cardinality of $\mathcal{J}_i^\text{TO}$ is $M_i = \binom{i+p}{p}$ which increases rapidly in $d$ and $p$, where $i = 1, \dots, d$. Smaller optimisation problems can be produced by constructing subsets of $\mathcal{J}_i^\text{TO}$. These index sets are discussed
%DIF >  in~\cite{parno2018transport}. Increased information with a slower increase in the number of map parameters can be achieved with the composition of maps discussed in~\cite{parno2015transport}.
...
in~\mbox{%DIFAUXCMD
\cite{parno2014transport}}\hspace{0pt}%DIFAUXCMD
. Increased information with a slower increase in the number of map parameters can be achieved with the composition of maps discussed in~\mbox{%DIFAUXCMD
\cite{parno2015transport}}\hspace{0pt}%DIFAUXCMD
...
Equation~\eqref{eq:gen_map_optim}. In \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , this
...
We now return to the structure of the map components given in \DIFdelbegin \DIFdel{Equation~}%DIFDELCMD < \eqref{eq:map_defn}%%%
...
these maps to propose new states. In \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend it was
...
adaptation is shown in~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . In this Section, we
...
%DIF >  \DontPrintSemicolon
%DIF >  \BlankLine
%DIF >  Initialise state $\theta^{(1)}_i = \theta_0$, \quad $i = 1,\dots,M$.\;
%DIF >  Initialise map $\bar{\gamma}^{(1)} = \iota$.\;
%DIF >  \For{$k \leftarrow 1, \dots, L-1$}{
%DIF >  	Compute $r_i = \tilde{T}(\theta^{(k)}_i; \bar{\gamma}^{(k)})$, \quad $i = 1,\dots,M$.\;
%DIF >  	Sample $r'_i \sim q_r(\cdot; r_i)$.\;
%DIF >  	Invert $\hat{\theta}_i^{(k)} = \tilde{T}^{-1}(r'_i; \bar{\gamma}^{(k)})$.\;
%DIF >  	Calculate:
%DIF >  	\begin{equation}
%DIF >  		w_i^{(k)} = \frac{\pi(\hat{\theta}_i^{(k)})}{\left(\sum_{j=1}^M \! q_r(r_i'; r_j)\right)|J_{\tilde{T}}(\hat{\theta}_i^{(k)};\bar{\gamma}^{(k)})|}.
%DIF >  	\end{equation}
\DIFaddbegin 

%DIF >  	Resample $\theta^{(k+1)} \leftarrow \|w^{(k)}\|^{-1}\sum\limits_{j=1}^M \! w_j^{(k)}\delta_{\hat{\theta}^{(k)}_j}(\cdot)$.\;

%DIF >  	\eIf{$k\ \text{mod}\ K_U = 0$ and $k < K_\text{stop}$}{
%DIF >  		\For{$i \leftarrow 1, \dots, n$}{
%DIF >  			Solve \eqref{eq:weighted_map_optim} with $\{(w^{(1)},\hat{\theta}^{(1)}), \dots, (w^{(k+1)},\hat{\theta}^{(k+1)})\}$
%DIF >  				and update $\gamma_i^{(k+1)}$.\;
%DIF >  		}
%DIF >  	}{
%DIF >  		$\bar{\gamma}^{(k+1)} = \bar{\gamma}^{(k)}$.\;
%DIF >  	}
%DIF >  }
%DIF >  \caption{ETAIS algorithm with adaptive transport map. Option 1.\label{alg:TransportETAIS1}}
%DIF >  \end{algorithm}
%DIF >  \end{table}
...
\DIFdelbeginFL %DIFDELCMD < \For{$k \leftarrow 1, \dots, L-1$}{
%DIFDELCMD < 	Compute $r_i = \tilde{T}(\theta^{(k)}_i; \bar{\gamma}^{(k)})$, \quad $i = 1,\dots,M$.\;
%DIFDELCMD < 	Sample $r'_i \sim q_r(\cdot; r_i)$.\;
%DIFDELCMD < 	Invert $\hat{\theta}_i^{(k)} = \tilde{T}^{-1}(r'_i; \bar{\gamma}^{(k)})$.\;
%DIFDELCMD < 	Calculate:
%DIFDELCMD < 	\[
%DIFDELCMD < 		w_i^{(k)} = \frac{\pi(\hat{\theta}_i^{(k)})}{\left(\sum_{j=1}^M \! q_r(r_i'; r_j)\right)|J_{\tilde{T}}(\hat{\theta}_i^{(k)};\bar{\gamma}^{(k)})|}.
%DIFDELCMD < 	\]
%DIFDELCMD < 

%DIFDELCMD < 	Resample $\theta^{(k+1)} \leftarrow \|w^{(k)}\|^{-1}\sum\limits_{j=1}^M \! w_j^{(k)}\delta_{\hat{\theta}^{(k)}_j}(\cdot)$.\;
%DIFDELCMD < 

%DIFDELCMD < 	\eIf{$k\ \text{mod}\ K_U = 0$ and $k < K_\text{stop}$}{
%DIFDELCMD < 		\For{$i \leftarrow 1, \dots, n$}{
%DIFDELCMD < 			Solve \eqref{eq:weighted_map_optim} with $\{(w^{(1)},\hat{\theta}^{(1)}), \dots, (w^{(k+1)},\hat{\theta}^{(k+1)})\}$
%DIFDELCMD < 				and update $\gamma_i^{(k+1)}$.\;
%DIFDELCMD < 		}
%DIFDELCMD < 	}{
%DIFDELCMD < 		$\bar{\gamma}^{(k+1)} = \bar{\gamma}^{(k)}$.\;
%DIFDELCMD < 	}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \For{$k \leftarrow 1, \dots, N-1$}{
	Compute $r_i = \tilde{T}(\theta^{(k)}_i; \bar{\gamma}^{(k)})$, \quad $i = 1,\dots,M$.\;
	Sample \edit{$\hat{r}_i \sim q_r(\cdot; r_i)$.\;
	Invert $\hat{\theta}_i^{(k)} = \tilde{T}^{-1}(\hat{r}_i; \bar{\gamma}^{(k)})$.\;
	Calculate:
	\begin{equation}
		w_i^{(k)} = \frac{\pi(\hat{\theta}_i^{(k)})}{\left(\sum_{j=1}^M \! q_r(\hat{r}_i; r_j)\right)|J_{\tilde{T}}(\hat{\theta}_i^{(k)};\bar{\gamma}^{(k)})|}.
	\end{equation}
...
\DIFaddendFL \caption{ETAIS algorithm with adaptive transport map. Option \DIFdelbeginFL \DIFdelFL{1.}%DIFDELCMD < \label{alg:TransportETAIS1}%%%
...
%DIFDELCMD < \BlankLine
%DIFDELCMD < %%%
\DIFdelFL{Initialise state $\theta^{(1)}_i = \theta_0$, }%DIFDELCMD < \quad %%%
\DIFdelFL{$i = 1,\dots,M$.\;
Initialise map $\bar{\gamma}^{(1)} = \iota$.\;
}%DIFDELCMD < \For{$k \leftarrow 1, \dots, N-1$}{
%DIFDELCMD < 	Compute $r_i = \tilde{T}(\theta^{(k)}_i; \bar{\gamma}^{(k)})$, \quad $i = 1,\dots,M$.\;
%DIFDELCMD < 	Sample $r'_i \sim q_r(\cdot; r_i)$.\;
%DIFDELCMD < 	Invert $\hat{\theta}_i^{(k)} = \tilde{T}^{-1}(r'_i; \bar{\gamma}^{(k)})$.\;
%DIFDELCMD < 	Calculate:
...
%DIFDELCMD < 	Resample $r^* \leftarrow \|w^{(k)}\|^{-1}\sum\limits_{j=1}^M \! w_j^{(k)}\delta_{r'_j}(\cdot)$.\label{algline:TETAIS_resample}\;
%DIFDELCMD < 	Invert $\theta^{(k+1)}_i = \tilde{T}^{-1}(r^*_i)$.\;
%DIFDELCMD < 	\eIf{$k\ \text{mod}\ K_U = 0$ and $k < K_\text{stop}$}{
%DIFDELCMD < 		\For{$i \leftarrow 1, \dots, n$}{
%DIFDELCMD < 			Solve \eqref{eq:weighted_map_optim} with $\{(w^{(1)},\hat{\theta}^{(1)}), \dots, (w^{(k+1)},\hat{\theta}^{(k+1)})\}$
%DIFDELCMD < 				and update $\gamma_i^{(k+1)}$.\;
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{ETAIS algorithm with adaptive transport map. Option 2.}%DIFDELCMD < \label{alg:TransportETAIS2}%%%
}
%DIFAUXCMD
...
region, although not necessarily outside\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{parno2014transport}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parno2018transport}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbeginFL %DIFDELCMD < \subfigure[Comparison of Transport ETAIS option (3) with the standard algorithms.]{\includegraphics[width=0.45\textwidth]{images/TPAIS/R1_L2_all}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfigure[Comparison of Transport ETAIS option 2 with the standard algorithms.]{\includegraphics[width=0.45\textwidth]{images/TPAIS/R1_L2_all}}
...
\DIFdelbegin \[\DIFdel{\alpha_j(X(t)) = \lim_{dt \to 0} \mathbb{P}(\text{Reaction $R_j$ in
    the time interval  } \quad s \in }[\DIFdel{t, t+ dt}] \DIFdel{).}\] %DIFAUXCMD
\DIFdel{If a system
satisfies what is called }\emph{\DIFdel{mass action kinetics}}%DIFAUXCMD
...
}\DIFdelend \begin{equation}\DIFdelbegin %DIFDELCMD < \label{eq:MAK}
%DIFDELCMD < %%%
...
}\DIFdelend %DIF >  If a system
%DIF >  satisfies what is called \emph{mass action kinetics}, then the form of
%DIF >  the function $\alpha_j$ is determined, up to a rate constant, by the
%DIF >  reactants involved in that reaction:
%DIF >  \begin{equation}\label{eq:MAK}
%DIF >  \alpha_j(X) = k_j \prod_{m=1}^{N_s} \prod_{n=0}^{\nu_{j,m} -1} (X_m - n),
%DIF >  \end{equation}
%DIF >  where $\nu_{j,m}$ is the $m$th component of the stoichiometric vector
%DIF >  $\nu_j$, $X_m$ is the $m$th component of the state vector $X$, and the
%DIF >  $k_j$ are rate constants.
...
}\DIFdelend %DIF >  However, if the system is stiff, i.e. there
%DIF >  are some reactions which are firing many times on a timescale for
%DIF >  which others are unlikely to fire at all, then trajectories can become
%DIF >  prohibitively expensive to simulate, since these methods simulate
%DIF >  every single reaction event with the same cost. In such a system, one
%DIF >  might employ multiscale methods in order to approximate trajectories
%DIF >  at a lower cost than the exact algorithms.
...
errors\mbox{%DIFAUXCMD
\cite{cotter2016error}}\hspace{0pt}%DIFAUXCMD
. Another approach is the
constrained multiscale algorithm (CMA)\mbox{%DIFAUXCMD
\cite{cotter2011constrained,cotter2016constrained}}\hspace{0pt}%DIFAUXCMD
, based in part on the
equation-free approach to multiscale computations\mbox{%DIFAUXCMD
\cite{kevrekidis2003equation,erban2006gene}}\hspace{0pt}%DIFAUXCMD
...
to the literature\mbox{%DIFAUXCMD
\cite{cotter2016constrained}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend %DIF >  One common approach is to induce the quasi-equilibrium approximation
%DIF >  QEA. This approximation makes the assumption that fast reactions
%DIF >  enter quasi-equilibrium on a timescale which is negligible with
%DIF >  respect to the timescale on which the slow dynamics in the system are
%DIF >  evolving. This amounts to taking the asymptotic limit that the
%DIF >  propensities of the ``slow'' reaction channels are equal to zero. This
%DIF >  allows us to sample approximate trajectories of the slow reactions
%DIF >  without the need to compute many fast reaction events.
...
}\DIFdelend %DIF >  Multiscale methods are not only of use when forward evaluations of the
%DIF >  dynamics are costly, but can also be of use where we attempt to solve
%DIF >  an inverse problem where the fast variables are unobservable. One
%DIF >  approach in this situation would be to integrate over all possible
%DIF >  trajectories of the fast variables, but this would almost always be
%DIF >  prohibitively expensive. Another approach would be to use an
%DIF >  appropriate multiscale approximation, so that the effective dynamics
%DIF >  of the slow variables can be approximate without the need to consider
%DIF >  the rapid fluctuations of the fast variables in the system.
...
%DIFAUXCMD
\DIFdel{The random variables defined in Equations~}%DIFDELCMD < \eqref{eqn:chem_time_dist} %%%
\DIFdel{and }%DIFDELCMD < \eqref{eqn:chem_react_dist} %%%
...
mass action kinetics given in }%DIFDELCMD < \eqref{eq:MAK}%%%
\DIFdel{.
}%DIFDELCMD < 
...
\DIFdel{Our choice of Gamma prior distributions with hyper-parameters $(a_i, b_i)$ results in the posterior distribution of the form
	...
	\prod_{i=1}^{N_r} \! \text{Gamma}(k_i; a_i, b_i) \notag }\\
		&\DIFdel{\propto \exp\Bigg\{\sum\limits_{i=1}^K \Bigg[
				\mathbf{K}_i\log \alpha_0(\mathbf{Y}_i; \mathbf{k}) - \mathbf{T}_i\alpha_0(\mathbf{Y}_i; \mathbf{k}) %DIF < \notag \\
%DIF < 		& \qquad\qquad\qquad
...
made, we arrive at approximate }\emph{\DIFdel{effective}} %DIFAUXCMD
...
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:RTCS}%%%
	...
	\prod_{i=1}^{n_r} \! \text{Gamma}(k_i; a_i, b_i) \notag }\\
		&\DIFdel{\propto \exp\Bigg\{\sum\limits_{i=1}^K \Bigg[
				\mathbf{K}_i\log
                  ...
                  \mathbf{T}_i \bar{\alpha}_0(\mathbf{Y}_i; \mathbf{k}) %DIF < \notag \\
%DIF < 		& \qquad\qquad\qquad
				+\sum_{j=1}^{n_r} \mathbf{R}_{ij}\log \bar{\mathbf{p}}_j(\mathbf{Y}_i; \mathbf{k})
			\Bigg]  \notag}\\
		&	\DIFdel{\qquad\qquad\qquad{} + \sum_{i=1}^{n_r} ((a_i-1)\log k_i - b_ik_i)\Bigg\}, %DIFDELCMD < \label{eq:Apos}%%%
...
first, \DIFdelbegin \DIFdel{which is of the form }%DIFDELCMD < \eqref{eqn:chem_posterior}%%%
...
posterior using \DIFdelbegin %DIFDELCMD < \eqref{eq:Apos} %%%
...
\begin{table}
\centering
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{|l|R{2em}R{2em}R{2em}R{2em}|}
%DIFDELCMD < 	\hline
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF >  \begin{tabular}{|l|R{2em}R{2em}R{2em}R{2em}|}
\DIFaddbeginFL 

\begin{tabular}{lrrrr} 
	\toprule
	\DIFaddendFL Parameter$\backslash$Dimension $i$ & 1 & 2 & 3 & 4  \\ \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \cmidrule\DIFaddFL{(lr)}{\DIFaddFL{1-5}} 
	\DIFaddendFL $\alpha_i$ & 150 & 5 & 5 & 3 \\
	$\beta_i$ & $\frac{15}{9}$ & $\frac{5}{12}$ & $\frac{5}{12}$ & $1$ \\ \DIFdelbeginFL %DIFDELCMD < \hline 
%DIFDELCMD < %%%
...
\caption{CMA approximation \DIFdelbeginFL %DIFDELCMD < \eqref{eq:Apos} %%%
...
%DIFDELCMD < 		\draw (A) \irregularcircle{1cm}{2.5mm};
%DIFDELCMD < 		\draw (B) \irregularcircle{1cm}{0.5mm};
%DIFDELCMD < 
...
%DIF >  	\centering
%DIF >  	\begin{tikzpicture}
%DIF >  		\pgfmathsetseed{1}
...
%DIFDELCMD < 		\draw (A) \irregularcircle{1cm}{2.5mm};
%DIFDELCMD < 		\draw (B) \irregularcircle{1cm}{1.1mm};
%DIFDELCMD < 		\draw (C) \irregularcircle{1cm}{0.5mm};
%DIFDELCMD < 
...
\DIFdelFL{Couplings between parameter space and reference
	...
	%DIFAUXCMD
%DIFDELCMD < \label{fig:chem_coupling}
...
$R$. When using the transport map proposal distribution, we prefix the proposal method with a T, e.g. MH-RW  and MH-TRW, as well as ETAIS-RW and ETAIS-TRW. }\DIFdelend %DIF >  		\draw (A) \irregularcircle{1cm}{2.5mm};
%DIF >  		\draw (B) \irregularcircle{1cm}{0.5mm};
\DIFaddbegin 
...
%DIF >  		\draw (A) \irregularcircle{1cm}{2.5mm};
%DIF >  		\draw (B) \irregularcircle{1cm}{1.1mm};
%DIF >  		\draw (C) \irregularcircle{1cm}{0.5mm};
...
%DIF >  		\node at (10,-1.5) {$r\in R \subset\mathbb{R}^d$};
%DIF >  	\end{tikzpicture}
%DIF >  	\caption{Couplings between parameter space and reference
%DIF >            when using the transport map with (bottom) and without (top)
%DIF >          a log preconditioner.}
%DIF >  	\label{fig:chem_coupling}
%DIF >  \end{figure}

%DIF >  Figure~\ref{fig:chem_coupling} (top) shows how we define a bijective map,
%DIF >  $\hat{T}$, between parameter space $\mathcal{X}$ and a reference space
%DIF >  $R$.
...
}\DIFdelend %DIF >  Therefore we also consider how these algorithms perform when the transport map
%DIF >  is preconditioned by the logarithmic function applied to the parameters. We choose to use this log
%DIF >  preconditioner since it converts our strictly positive parameter
%DIF >  space, $\mathcal{X}$, into an intermediate space $\mathcal{Y} \subset
%DIF >  \mathbb{R}^d$. This allows us to define $T$ between two subsets of
%DIF >  $\mathbb{R}^d$, which means that all proposals made on the reference
%DIF >  space are mapped to valid
%DIF >  proposals on the true parameter space. It also reduces the work required of the map, which since
%DIF >  we use only a finite dimensional approximation, can improve
%DIF >  performance and stability.

\DIFdelbegin \DIFdel{Figure~\ref{fig:chem_coupling} (bottom) displays the composition of maps
when we include this $\log$ preconditioner leading to an intermediate space $\mathcal{Y}$. The inclusion of this additional map }\DIFdelend %DIF >  Figure~\ref{fig:chem_coupling} (bottom) displays the composition of maps
%DIF >  when we include this $\log$ preconditioner leading to an intermediate space $\mathcal{Y}$. 
\DIFaddbegin 
...
approximation of the posterior \DIFdelbegin %DIFDELCMD < \eqref{eq:Apos} %%%
...
\toprule
	...
	\DIFdelbeginFL \DIFdelFL{RW }%DIFDELCMD < & & %%%
\DIFdelFL{5.5e-3 }%DIFDELCMD < & & %%%
\DIFdelFL{1.0e-0 }%DIFDELCMD < & %%%
\DIFdelFL{9.0e-3 }%DIFDELCMD < \\
%DIFDELCMD < 	%%%
\DIFdelFL{TRW }%DIFDELCMD < & & %%%
\DIFdelFL{1.2e-0 }%DIFDELCMD < & & %%%
\DIFdelFL{4.0e-1 }%DIFDELCMD < & %%%
\DIFdelFL{1.2e-3 }%DIFDELCMD < \\
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF >  RW & & 5.5e-3 & & 1.0e-0 & 9.0e-3 \\
	%DIF >  TRW & & 1.2e-0 & & 4.0e-1 & 1.2e-3 \\
...
\begin{figure}[!htb]
\centering
\DIFdelbeginFL %DIFDELCMD < \subfigure[Sampling algorithms with no preconditioner for $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/Applications/CMA_L2_param_space"}}
%DIFDELCMD < \subfigure[Sampling algorithms with a $\log$ preconditioner for $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/Applications/CMA_L2_log_space"}}
%DIFDELCMD < %%%
\DIFdelendFL %DIF >  \subfigure[Sampling algorithms with no preconditioner for
    %DIF >      $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/Applications/CMA_L2_param_space"}}
...
% logTRW being apparently slightly less stable. 
...
\centering
\DIFdelbeginFL %DIFDELCMD < \subfigure[Push forward of the posterior sample through $\hat{T}$ with
%DIFDELCMD < no log
%DIFDELCMD < preconditioner.]{\includegraphics[width=0.8\textwidth]{"images/Applications/CMA_ref_space_MHTRW"}}
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelendFL %DIF >  \subfigure[Push forward of the posterior sample through $\hat{T}$ with
%DIF >  no log
%DIF >  preconditioner.]{\includegraphics[width=0.8\textwidth]{"images/Applications/CMA_ref_space_MHTRW"}}
%DIF >  \\
...
\DIFaddbegin \edit{At this point, we note that if the transport map found is equal
  ...
  from the push forward of the target distribution. % ****HIGHLIGHT THIS
  % POINT ELSEWHERE -DEFINITELY IN THE CONCLUSIONS AND MAYBE THE INTRO/ABSTRACT?*****
  }

\DIFaddend \subsubsection{Comparison of the Constrained and QEA approaches}
%DIF >  ****GREATLY REDUCE THIS SECTION****
%DIF >  ****Jump straight to Bayes' factors and Figure 7.5****
...
\DIFdelend \DIFaddbegin \DIFadd{.
  %DIF >  , as can be seen in
%DIF >  Figure~\ref{fig:chem_diff}, in which we plot the difference in the 
%DIF >  posteriors densities
%DIF >  \begin{equation}\label{eqn:chem_diff}\pi^{\text{CMA}}(\mathbf{k}|\mathbf{R},\mathbf{T}) - \pi^{\text{QEA}}(\mathbf{k}|\mathbf{R},\mathbf{T}).
%DIF >  \end{equation}
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.9\textwidth]{"images/Applications/Diff_CMA_QEA"}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Difference between the CMA and QEA posteriors as defined in Equation~}%DIFDELCMD < \eqref{eqn:chem_diff}%%%
\DIFdelFL{.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:chem_diff}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{figure}[htb]
%DIF >  \centering
%DIF >  \includegraphics[width=0.9\textwidth]{"images/Applications/Diff_CMA_QEA"}
%DIF >  \caption{Difference between the CMA and QEA posteriors as defined in Equation~\eqref{eqn:chem_diff}.}
%DIF >  \label{fig:chem_diff}
%DIF >  \end{figure}
...
}\DIFdelend %DIF >  Since the two posteriors have been approximated using an MCMC sample,
%DIF >  there is a visible amount of Monte Carlo static, particularly in the tails of
%DIF >  the distributions. We can see that the differences in the marginals
%DIF >  for $k_1, k_2,$ and $k_3$ are relatively small, but the marginal for
%DIF >  $k_4$ varies by a significant amount. 
...
model, with all reactions (including fast reactions) observed.
    ...
    %DIF >      The Bayes factor,
%DIF >  $B_{1,2}$, between two models, $\mathcal{M}_1$ and $\mathcal{M}_2$,
%DIF >  given data $D$, can be interpreted as a ratio of the normalisation constants of the posterior distributions given each model,
%DIF >  \begin{equation}
%DIF >  	B_{1,2} = \frac{\mathbb{P}(D|\mathcal{M}_1)}{\mathbb{P}(D|\mathcal{M}_2)}, \quad \text{where} \quad \mathbb{P}(D|\mathcal{M}_k) = \int_\mathcal{X} \mathbb{P}(D|\theta_k,\mathcal{M}_k)\mathbb{P}(\theta_k|\mathcal{M}_k) \, \text{d}\theta_k.
%DIF >  \end{equation}
%DIF >  Under the ETAIS framework, it is straightforward to calculate these factors using the Monte Carlo estimator for the normalisation constants. From~\cite{robert2013monte} these normalisation constants take the form
%DIF >  \begin{equation}
%DIF >  	Z_k \approx \hat{Z}_k\frac{1}{NM}\sum_{i=1}^N\sum_{j=1}^M w_j^{(i)}(\mathcal{M}_k),
%DIF >  \end{equation}
%DIF >  where $w_j^{(i)}(\mathcal{M}_k)$ is the weight under model $\mathcal{M}_k$ corresponding to the $j$-th ensemble member on the $i$-th iteration. Hence, $B_{1,2} \approx \hat{Z}_1/\hat{Z}_2$. We can compare more than two models in this way by selecting the model with the largest marginal distribution as the best model.
\DIFaddbegin 

%DIF >  We now label the constrained model as $\mathcal{M}_1$, the model
%DIF >  arising from the QEA as $\mathcal{M}_2$, and the full data model as
%DIF >  $\mathcal{M}_0$. Model $\mathcal{M}_1$ is dependent on the parameter
%DIF >  $\theta_1 = (k_1, \hat{k}_4^{\text{CMA}})^\top$, and model
%DIF >  $\mathcal{M}_2$ is dependent on the parameters $\theta_2 = (k_1,
%DIF >  \hat{k}_4^{\text{QEA}})^\top$. The full model $\mathcal{M}_0$ is
%DIF >  dependent on all four parameters $\theta_0 = (k_1, k_2, k_3, k_4)^\top$. 
...
%DIFAUXCMD
\DIFdel{Under the ETAIS framework, it is straightforward to calculate these factors using the Monte Carlo estimator for the normalisation constants. From~\mbox{%DIFAUXCMD
\cite{robert2013monte} }\hspace{0pt}%DIFAUXCMD
...
\begin{tabular}{cccc}
  ...
  & \DIFdelbeginFL \DIFdelFL{$B_{0,k}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bayes' factor  }\DIFaddendFL \\ \cmidrule(lr){1-4}
	Full & 0 & 6.8e-3 & 1 \\
	CMA & 1 & 3.2e-3 & 2.09 \\
...
\caption{Marginal distributions and Bayes factors for the data
  ...
  Section~\ref{sec:chem_multiscale}. \DIFaddbeginFL \DIFaddFL{The Bayes' factors are computed
  with respect to the ``full'' model.}\DIFaddendFL }
\label{tab:chem_Bayes_marginals}
\end{table}

Computing the Bayes factors from Table~\ref{tab:chem_Bayes_marginals}
we see that we should of course prefer the model \DIFdelbegin \DIFdel{$\mathcal{M}_0$ in
which we observe all reactions perfectly}\DIFdelend \DIFaddbegin \edit{ in
which we observe all reactions perfectly and are able to explicitly
compute the likelihood}\DIFaddend . However this model is not
...
Equation~}%DIFDELCMD < \eqref{eqn:chem_posterior}%%%
...
\begin{table}[!h]
\centering
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{|l|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|}
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > \begin{tabular}{|l|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|R{1.8em}|}
...
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{lrrrr}
	\DIFaddendFL \toprule
		 & \DIFdelbeginFL %DIFDELCMD < \multicolumn{4}{c}{MH} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c}{MH} \DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \multicolumn{4}{c}{ETAIS} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c}{ETAIS} \DIFaddendFL \\ \cmidrule(lr){\DIFdelbeginFL \DIFdelFL{2-5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2-3}\DIFaddendFL } \cmidrule(lr){\DIFdelbeginFL \DIFdelFL{6-9}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4-5}\DIFaddendFL }
		& \DIFdelbeginFL \DIFdelFL{RW }%DIFDELCMD < & %%%
\DIFdelendFL logRW & \DIFdelbeginFL \DIFdelFL{TRW }%DIFDELCMD < & %%%
\DIFdelendFL log-TRW & \DIFdelbeginFL \DIFdelFL{RW }%DIFDELCMD < & %%%
\DIFdelendFL logRW & \DIFdelbeginFL \DIFdelFL{TRW }%DIFDELCMD < & %%%
\DIFdelendFL log-TRW \\ \midrule
		$\beta_{\%}^*$	 	& \DIFdelbeginFL \DIFdelFL{1.0e-1 }%DIFDELCMD < & %%%
\DIFdelendFL 3.9e-1 &\DIFdelbeginFL \DIFdelFL{1.0e-0 }%DIFDELCMD < &%%%
\DIFdelendFL 6.0e-1 & - & - \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelFL{- }%DIFDELCMD < & %%%
\DIFdelFL{- }\DIFdelendFL \\
		$\beta_{\text{ESS}}^*$	       & -            & -          & \DIFdelbeginFL \DIFdelFL{-            }%DIFDELCMD < & %%%
\DIFdelFL{-            }%DIFDELCMD < & %%%
\DIFdelFL{1.0e-0 }%DIFDELCMD < & %%%
\DIFdelendFL 1.3e-0 & \DIFdelbeginFL \DIFdelFL{8.0e-1 }%DIFDELCMD < & %%%
\DIFdelendFL 5.0e-1 \\
		ESS/$M$		 	 & -            & -            & \DIFdelbeginFL \DIFdelFL{-            }%DIFDELCMD < & %%%
\DIFdelFL{-            }%DIFDELCMD < & %%%
\DIFdelFL{3.5e-4 }%DIFDELCMD < & %%%
\DIFdelendFL 4.7e-2 & \DIFdelbeginFL \DIFdelFL{4.8e-2 }%DIFDELCMD < & %%%
\DIFdelendFL 1.6e-1  \\
	\bottomrule
	\end{tabular}
...
\begin{figure}[!htb]
\centering
\DIFdelbeginFL %DIFDELCMD < \subfigure[Sampling algorithms with no preconditioner for $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/Applications/GRN_convergence"}}
%DIFDELCMD < \subfigure[Sampling algorithms with a $\log$ preconditioner for $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/Applications/GRN_convergence_log"}}
%DIFDELCMD < %%%
\DIFdelendFL %DIF >  \subfigure[Sampling algorithms with no preconditioner for $\tilde{T}$.]{\includegraphics[width=0.495\textwidth]{"images/Applications/GRN_convergence"}}
...
}\DIFdelend %DIF >  We note that in this 
%DIF >  example the ETAIS-TRW algorithm does not perform as well as the ETAIS-RW
%DIF >  algorithm. This is due to the lack of log preconditioner. The
%DIF >  posterior has support only on the positive quadrant, and without the
%DIF >  preconditioner, the transport map has to work hard to enforce
%DIF >  this. With a relatively low-dimensional approximation being used for
%DIF >  the transport map, this makes it more likely that the map produced
%DIF >  may hinder sampling rather than improving it. This highlights the
%DIF >  importance of using preconditioners with transport maps in order to
%DIF >  ensure absolute continuity of the posterior distribution with respect
%DIF >  to the pullback of the proposal distribution on reference space.
...
the map in higher dimensions\mbox{%DIFAUXCMD
\cite{whitney_self-intersections_1994,nash1956imbedding,takens_detecting_1981}}\hspace{0pt}%DIFAUXCMD
...
}\DIFdelend %DIF >  \subsection{Invertibility of the transport map}
%DIF >  In this paper, we have considered an approach similar to that
%DIF >  presented in \cite{parno2018transport}, where we aim to find an
%DIF >  invertible map which approximates the true transport map between the
%DIF >  reference measure and the target measure. However, this true map is
%DIF >  invariably itself not invertible. However, by considering embedding
%DIF >  the map in higher dimensions\cite{whitney_self-intersections_1994,nash1956imbedding,takens_detecting_1981}, these issues of non-invertibility may be
%DIF >  avoided, and higher quality maps may be found which better aid
%DIF >  exploration and sampling of the whole state space.
